...
%                     9 : A straight line (GS): x(t) = y(t) = z(t) = t
...
elseif data_flag == 8 % A helix (GS)
    fprintf('Helix \n');
    par = 100;   % controls scale
    par2 = .05;   % controls the speed of helix revolution 
                 % (less: tends to straight line, greater: tends to circle)
    tt = par*rand(1,N);
    height = tt;
    Y = [height; par*cos(par2*tt); par*sin(par2*tt)];
    X =  [tt; tt];
elseif data_flag == 9 % A straight line (GS)
    fprintf('Straight line \n');
    tt = 1000*rand(1,N);
    height = tt;
    Y = [tt+randn(size(tt)); height+randn(size(tt)); tt+randn(size(tt))];
    X =  [tt; tt];    
else
